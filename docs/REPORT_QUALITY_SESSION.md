...
> **Scope:** Make report generation the crux of the product — genuinely consulting-grade content, an honest docs-only vs interview story, LLM-written narrative + agentic ideas, a real consultant review/edit loop with live preview, an in-app viewer for the company, and light-touch agent attribution. Plus adjacent platform fixes (catalog products, audit log, Operations consolidation).
...
1. **Phases 1–4 + catalog rework** — the initial report-quality pass (credibility, the LLM writer, consultant editorial control), a course-correction on the "owned products" feature, and platform-admin fixes.
...
- **The stored snapshot is immutable.** Consultant edits and previews composite onto a *copy* of `report.report_snapshot` at render time, so the AI-generated body and expert edits stay separable and auditable.
...
| 3 | `6e3f8fa` | Report Phase 3: consultant editorial control over the deliverable |
...
| 9 | `c94f5c2` | Platform product catalog: first-party flag + consultant add-from-catalog |
...
| 13 | `e22cca4` | Report WS1: live WYSIWYG preview with pending consultant edits |
| 14 | `ac9b159` | Report WS2: in-app report viewer for the company + freshness states |
| 15 | `e9804bd` | Report WS5: harden consultant submit gate + one-click edit preview |
...
- **Consultant edits** (`ReportSectionOverride`) and consultant notes/findings are applied at **regenerate / preview** time via `Reports::SectionOverridesApplier` + `Reports::ReviewNotesCollector` — never mutating the stored snapshot.
- **Platform approval** triggers `Reports::RegenerateWithReviewService`, which re-renders the stored snapshot **with** the consultant overlay and re-uploads the artifact.
...
- **Surfaced real consultant credentials** in the validation appendix (was a hardcoded "Expert consultant").
...
## 5. Phase 3 — Consultant editorial control (`6e3f8fa`)

**Problem:** Consultants could only append an appendix — they had no control over the report body.

**What we did:**
- New model **`ReportSectionOverride`** (`hide` / `edit` / `add`), with a consultant CRUD endpoint.
...
- Consultant workspace "Report sections" step gained a **Section editor** panel (hide toggles, add editorial note, add a whole custom section anchored after any built-in section, with consultant attribution).
...
Originally implemented a **company-facing** "Owned solutions" page (the *company* registers what they own), a description-aware fit service, a report section, and consultant endorsement. Also added the **department × friction heatmap** exhibit (kept).

### 6b. The correction (`a80da0a` revert, `c94f5c2` rebuild)
Stakeholder clarified the real requirement: **the platform** uploads *their* products (a mix of first-party built products + curated third-party tools); the system assesses fit per company during analysis/report; **consultants** can add products to a company's list; the **company admin does not** manage this.
...
**Problem (the biggest UX hole):** consultants authored section edits **blind** — the "preview" only showed the already-generated stored PDF, and the platform approver approved edits sight-unseen.
...
- New consultant + platform `preview` endpoints return that live HTML.
- Consultant report drawer gained a **"With your edits" / "Stored PDF"** toggle, refetched each open so it reflects the latest overrides.
- Platform approval panel gained a **"With consultant edits" / "Current artifact"** toggle so approval is no longer sight-unseen.
...
## 13. WS5 — Consultant submit gate + edit preview (`e9804bd`)

- **Disable "Submit review"** until the consultant's own requirements are met (all sections reviewed + overall conclusion saved), with a hint — the gate was previously soft (always enabled).
...
- **New specs:** `narrative_writer_spec`, `agentic_idea_writer_spec`, `section_overrides_applier_spec`, `owned_solution_fit_service_spec` (from the reverted slice), consultant `catalog_add_spec`, plus the report service specs.
...
- **Unifying catalog + agentic endorsement into the consultant workspace** (still lives on separate consultant pages).
