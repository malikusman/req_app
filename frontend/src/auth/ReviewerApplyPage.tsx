...
export function ReviewerApplyPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      await api.publicReviewerApplication({
        name,
        email,
  ...
  return (
    ...
    >
      <div className="mb-6">
        <h1 className="font-display text-page-title text-text-primary m-0">Become a reviewer</h1>
        <p className="mt-1 text-sm text-text-secondary">
          Already approved? <Link to="/reviewer/login">Sign in</Link>
        </p>
      </div>
