/** Consultant profile questionnaire — 9 soft sections, mirrors company onboarding UX. */

export type ConsultantFieldType =
  | 'single_select'
  | 'multi_select'
  | 'searchable_select'
  | 'text'
...
export type ConsultantQuestionnaireField = {
  id: string;
  type: ConsultantFieldType;
  label: string;
  options?: string[];
...
export type ConsultantQuestionnaireSection = {
  id: number;
  title: string;
  shortTitle: string;
  fields: ConsultantQuestionnaireField[];
};

export const CONSULTANT_INDUSTRY_OPTIONS = [
  'Retail & E-commerce',
  'Manufacturing',
...
export const CONSULTANT_LOCATION_OPTIONS = [
...
export const CONSULTANT_STRENGTH_OPTIONS = [
...
export const CONSULTANT_QUESTIONNAIRE_SECTIONS: ConsultantQuestionnaireSection[] = [
  {
    id: 1,
    ...
    fields: [
      ...
      { id: 'website', type: 'text', label: 'Website (optional)', placeholder: 'https://…' },
      {
        ...
        label: 'Location',
        options: CONSULTANT_LOCATION_OPTIONS,
      },
    ],
  },
  {
    ...
    fields: [
      {
        ...
        options: CONSULTANT_INDUSTRY_OPTIONS,
      },
      {
    ...
    fields: [
      {
        ...
        maxSelections: 6,
        options: CONSULTANT_STRENGTH_OPTIONS,
      },
      {
        id: 'review_focus',
        type: 'multi_select',
...
export type ConsultantAnswers = Record<string, string | string[] | unknown[] | boolean | undefined>;

export function computeConsultantCompletionPercent(
  answers: ConsultantAnswers,
  extras?: { hasAvatar?: boolean; hasCv?: boolean; hasExperiences?: boolean }
): number {
  const ids = CONSULTANT_QUESTIONNAIRE_SECTIONS.flatMap((s) => s.fields.map((f) => f.id));
...
export function consultantSectionTouched(sectionId: number, answers: ConsultantAnswers, extras?: {
  hasAvatar?: boolean;
  hasCv?: boolean;
  hasExperiences?: boolean;
}): boolean {
  const section = CONSULTANT_QUESTIONNAIRE_SECTIONS.find((s) => s.id === sectionId);
