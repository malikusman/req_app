...
export function ReviewerDocuments() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api
      .reviewerDocuments(token, Number(companyId))
  ...
  const onDownload = async (doc: CompanyDocument) => {
    ...
    setError('');
    try {
      await api.downloadReviewerDocument(token, Number(companyId), doc.id, doc.filename);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Internal docs shared with reviewers for evidence and context."
        breadcrumbs={[
        ...
        <DataTable
          ...
          emptyState={
            <EmptyState
              ...
              description="When the company uploads documents marked visible to reviewers, they appear here."
            />
          }
        />
