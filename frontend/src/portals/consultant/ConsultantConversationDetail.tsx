...
export function ConsultantConversationDetail() {
  ...
  const load = () => {
    ...
    return api.consultantConversation(token, Number(companyId), Number(conversationId)).then((d) => {
      setMessages(
        d.messages.map((m) => ({
          id: m.id,
  ...
  const sendFollowup = async (e: FormEvent) => {
    ...
    setSending(true);
    try {
      await api.sendConsultantFollowup(token, Number(companyId), employeeId, followupBody.trim());
      setFollowupBody('');
      await load();
    } finally {
      setSending(false);
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: 'Conversations', href: `/consultant/companies/${companyId}/conversations` },
          { label: 'Transcript' },
        ]}
      />
