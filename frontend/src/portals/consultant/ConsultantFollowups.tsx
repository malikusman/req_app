...
import { api, type AppNotification, type ConsultantFollowupRow } from '../../lib/api';
...
export function ConsultantFollowups() {
  const token = useConsultantToken();
  const [tab, setTab] = useState<InboxTab>('followups');
  const [followups, setFollowups] = useState<ConsultantFollowupRow[]>([]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError('');
    Promise.all([api.consultantFollowups(token), api.consultantNotifications(token)])
  ...
  const markAllRead = async () => {
    if (!token) return;
    await api.markAllConsultantNotificationsRead(token);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Inbox"
        description="Employee follow-ups, co-consultant mentions, and notifications."
        breadcrumbs={[{ label: 'Dashboard', href: '/consultant/dashboard' }, { label: 'Inbox' }]}
        actions={
          unreadCount > 0 ? (
      ...
      {tab === 'notifications' && (
        <>
          {notifications.length === 0 ? (
            <EmptyState title="No notifications" description="Co-consultant messages and discussion mentions will appear here." />
          ) : (
            <div className="space-y-3">
              {notifications.map((n) => (
                <Card key={n.id}>
