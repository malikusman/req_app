...
export function ConsultantDashboard() {
  ...
  const companiesTo = firstCompany ? `/consultant/companies/${firstCompany.id}` : '/consultant/profile';

  const firstName = (session?.portal === 'consultant' ? session.user.name : '').trim().split(/\s+/)[0] || 'there';
  ...
  const hero = usedHeroReview
    ? {
        ...
        primaryAction: {
          label: 'Open review',
          to: `/consultant/companies/${heroReviewItem!.company_id}/reports/${heroReviewItem!.report_id}/review`,
        },
      }
    : stats.open_followups > 0
      ? {
          ...
          primaryAction: { label: 'Open inbox', to: '/consultant/inbox' },
        }
      : {
          ...
          primaryAction: firstCompany
            ? { label: 'Open a company', to: `/consultant/companies/${firstCompany.id}` }
            : { label: 'View your profile', to: '/consultant/profile' },
        };
  ...
  const attentionItems: AttentionItemData[] = [
    ...data.attention_items
      ...
      .map<AttentionItemData>((i) => ({
        ...
        action: {
          label: 'Review',
          to: `/consultant/companies/${i.company_id}/reports/${i.report_id}/review`,
        },
      })),
      ...
      ? [
          {
            ...
            detail: 'Co-consultant and platform updates',
            action: { label: 'Open inbox', to: '/consultant/inbox' },
          },
        ]
      : []),
  ];
  ...
  const pendingReviewTo = usedHeroReview
    ? `/consultant/companies/${heroReviewItem!.company_id}/reports/${heroReviewItem!.report_id}/review`
    : companiesTo;
    ...
    <div className="space-y-6">
      ...
      {data.profile.profile_status !== 'published' && data.profile.profile_completeness_percent != null && (
        <div className="rounded-md border border-accent/30 bg-accent-muted px-4 py-3 text-sm text-foreground">
          ...
          <Link to="/consultant/profile" className="font-semibold text-accent-hover hover:underline">
