...
export function ConsultantAnnotationRail({
  activeSection,
  onSectionChange,
  sectionStates,
  sectionComments,
  ...
  coConsultantReviews: CoConsultantReview[];
  submitted: boolean;
  commentBody: string;
  ...
  return (
    <aside className="flex h-full min-h-0 flex-col gap-4">
        ...
        <ul className="mb-4 max-h-48 space-y-2 overflow-y-auto">
          {filteredComments.length === 0 ? (
            ...
            filteredComments.map((c) => {
              const isMine = currentConsultantUserId != null && c.consultant_user_id === currentConsultantUserId;
              ...
              return (
                ...
                >
                  <div className="mb-1 flex flex-wrap items-center gap-2">
                    <p className="m-0 text-xs text-muted-foreground">{c.consultant_name}</p>
      ...
      {coConsultantReviews.length > 0 && (
        <Card title="Co-consultant progress" className="min-h-0 flex-1 overflow-y-auto">
          {coConsultantReviews.map((cr) => {
            const activity = cr.activity || cr.status;
            return (
              <div key={cr.consultant_name} className="border-t border-border py-3 first:border-0 first:pt-0">
