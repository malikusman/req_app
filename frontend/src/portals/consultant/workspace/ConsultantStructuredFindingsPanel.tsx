...
export function ConsultantStructuredFindingsPanel({
  companyId,
  reportId,
  readOnly,
}: {
  ...
  const load = () => {
    if (!token) return;
    api
      .consultantReportFindings(token, companyId, reportId)
  ...
  useEffect(() => {
    if (!token) return;
    Promise.all([api.consultantSignals(token, companyId), api.consultantPatterns(token, companyId)])
  ...
  const create = async () => {
    ...
    setError('');
    try {
      await api.createConsultantReportFinding(token, companyId, reportId, {
