...
import { api, type ConsultantExperience, type ConsultantProfile, type ConsultantPublicCard } from '../../lib/api';
...
import {
  CONSULTANT_QUESTIONNAIRE_SECTIONS,
  computeConsultantCompletionPercent,
  consultantSectionTouched,
  type ConsultantAnswers,
  type ConsultantQuestionnaireField,
} from '../../lib/consultantQuestionnaireOptions';
...
const emptyExperience = (): ConsultantExperience => ({
  organization: '',
  title: '',
...
function SearchableSelectField({
  ...
  field: ConsultantQuestionnaireField;
  value: string;
...
export function ConsultantProfile() {
  ...
  const hydrateFromProfile = (d: Awaited<ReturnType<typeof api.consultantProfile>>) => {
    setProfile(d.profile);
    const qa = (d.questionnaire_answers || {}) as ConsultantAnswers;
    const yearsBand =
    ...
    const merged: ConsultantAnswers = {
      name: d.user.name,
      email: d.user.email,
      ...
      experiences:
        (qa.experiences as ConsultantExperience[])?.length
          ? (qa.experiences as ConsultantExperience[])
    ...
    setPercent(
      d.completion_percent ??
        computeConsultantCompletionPercent(merged, {
  ...
  useEffect(() => {
    ...
    api
      .fetchConsultantAvatarPreview(token, profile.avatar_url)
      .then((url) => {
        if (cancelled) {
  ...
  const setAnswer = (id: string, value: unknown) => {
    setAnswers((prev) => {
      const next = { ...prev, [id]: value as ConsultantAnswers[string] };
      setPercent(computeConsultantCompletionPercent(next, extras));
      return next;
    });
  };

  const section = useMemo(
    () => CONSULTANT_QUESTIONNAIRE_SECTIONS.find((s) => s.id === sectionId) || CONSULTANT_QUESTIONNAIRE_SECTIONS[0],
    [sectionId]
  );

  const isPublished = profile?.profile_status === 'published';

  const companyPreview = useMemo((): ConsultantPublicCard => {
    const expRows = (
      Array.isArray(answers.experiences) ? (answers.experiences as ConsultantExperience[]) : profile?.experiences || []
  ...
  const persist = async (nextSection?: number, opts?: { quiet?: boolean }) => {
    ...
    setError('');
    try {
      const res = await api.updateConsultantQuestionnaire(token, {
  ...
  const publish = async () => {
    ...
    setError('');
    try {
      await api.updateConsultantQuestionnaire(token, {
        questionnaire_answers: answers as Record<string, unknown>,
        questionnaire_step: sectionId,
      });
      const res = await api.updateConsultantProfile(token, { publish: true });
      setProfile(res.profile);
      toast({
        variant: 'success',
  ...
  const onAvatar = async (file: File | undefined) => {
    ...
    try {
      const res = await api.uploadConsultantAvatar(token, file);
      setProfile(res.profile);
      setPercent(
        computeConsultantCompletionPercent(answers, {
          hasAvatar: true,
  ...
  const onCv = async (file: File | undefined) => {
    if (!token || !file) return;
    try {
      const res = await api.uploadConsultantCv(token, file);
      setProfile(res.profile);
  ...
  const renderField = (field: ConsultantQuestionnaireField) => {
    if (field.type === 'photo') {
      return (
        <div className="space-y-2">
          <p className="m-0 text-sm font-medium text-foreground">{field.label}</p>
    ...
    if (field.type === 'experiences') {
      const rows = (Array.isArray(answers.experiences) ? answers.experiences : [emptyExperience()]) as ConsultantExperience[];
  ...
  const sectionNav = (
    <nav className={cn(isNarrow ? 'flex gap-2 overflow-x-auto pb-1' : 'space-y-1')} aria-label="Profile sections">
      {CONSULTANT_QUESTIONNAIRE_SECTIONS.map((s) => {
        const touched = consultantSectionTouched(s.id, answers, extras);
    ...
    <div className="mx-auto flex min-h-[70vh] max-w-5xl flex-col gap-4 pb-28 lg:pb-8">
      ...
      >
        <div className="space-y-3">
          ...
          <ExpertConsultantCard consultant={companyPreview} token={token} />
        </div>
      </Modal>
        ...
        <Card className="space-y-6 p-4 sm:p-6">
          <div>
            <p className="m-0 text-xs font-medium uppercase tracking-wide text-muted-foreground">
              Section {section.id} of {CONSULTANT_QUESTIONNAIRE_SECTIONS.length}
            </p>
