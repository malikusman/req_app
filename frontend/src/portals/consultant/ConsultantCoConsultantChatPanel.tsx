...
export function ConsultantCoConsultantChatPanel({
  companyId,
  active,
  onMessagesLoaded,
}: {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = await api.consultantChatMessages(token, companyId);
  ...
  const send = async (e: FormEvent) => {
    ...
    setSending(true);
    try {
      await api.sendConsultantChat(token, companyId, body.trim());
      setBody('');
      await load();
    } finally {
      setSending(false);
    ...
    <div className="flex h-full min-h-0 flex-col">
      ...
      <form onSubmit={send} className="mt-4 shrink-0 space-y-3 border-t border-border pt-4">
        <Textarea
          ...
          placeholder="Message co-consultants…"
        />
