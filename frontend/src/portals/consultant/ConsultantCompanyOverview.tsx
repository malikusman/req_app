...
import { ConsultantChatDrawer } from './workspace/ConsultantChatDrawer';
...
export function ConsultantCompanyOverview() {
  const { companyId } = useParams();
  const token = useConsultantToken();
  const [company, setCompany] = useState<ConsultantCompanyDetail | null>(null);
  ...
  useEffect(() => {
    ...
    setError('');
    Promise.all([
      api.consultantCompany(token, Number(companyId)),
      api.consultantConversations(token, Number(companyId)).catch(() => ({ conversations: [] })),
      api.consultantEmployees(token, Number(companyId)).catch(() => ({ employees: [] })),
      api.consultantSignals(token, Number(companyId)).catch(() => ({ signals: [] })),
      api.consultantPatterns(token, Number(companyId)).catch(() => ({ patterns: [] })),
      api.consultantRecommendations(token, Number(companyId)).catch(() => ({ recommendations: [] })),
    ])
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={company.name}
        description="Everything shared by this company — the report to review, interviews, and your co-consultants."
        breadcrumbs={[{ label: 'Dashboard', href: '/consultant/dashboard' }, { label: company.name }]}
      />
      ...
      {tab === 'overview' && (
        <div className="space-y-6">
          <div className="grid gap-4 md:grid-cols-3">
            ...
            />
            <StatCard
              label="Co-consultants"
              value={company.co_consultant_count}
              ...
              <Card title="Evidence explorer">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Link to={`/consultant/companies/${companyId}/analysis`}>
                      <Button variant="secondary" icon={<ClipboardCheck className="h-4 w-4" />}>
                        Analysis
                      </Button>
                    </Link>
                    <Link to={`/consultant/companies/${companyId}/catalog`}>
            ...
            <div className="space-y-6">
              <Card title="Quick actions">
                <div className="flex flex-col gap-2">
                  <Link to={`/consultant/companies/${companyId}/conversations`} className="w-full">
                    <Button
                      variant="secondary"
                  ...
                  <Link to="/consultant/inbox" className="w-full">
                    <Button
                      variant="secondary"
                      className="w-full justify-start"
              ...
              <Card title="Collaboration">
                {hasCoConsultants ? (
                  <p className="m-0 text-sm text-muted-foreground">
                    {company.co_consultant_count} co-consultant{company.co_consultant_count === 1 ? '' : 's'}{' '}
                    on {company.name}. Use co-consultant chat to stay aligned before you submit.
                  </p>
                ) : (
                  <p className="m-0 text-sm text-muted-foreground">
                    You’re the only consultant assigned to {company.name}. A second consultant can be added
          ...
          <Card
            ...
            action={
              conversations.length > 0 ? (
                <Link
                  to={`/consultant/companies/${companyId}/conversations`}
                  ...
                  return (
                    <li key={employee.id} className="rounded-lg border border-border px-3 py-2.5">
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-3 text-sm">
                        <Link
                          to={`/consultant/companies/${companyId}/employees/${employee.id}/followup`}
                        ...
                        {conversation ? (
                          <Link
                            to={`/consultant/companies/${companyId}/conversations/${conversation.id}`}
                            className="font-medium text-accent hover:underline"
                          >
                            Transcript
                          </Link>
      ...
      {tab === 'ideas' && (
        <Card title="Agentic AI ideas">
          ...
          {token ? (
            <AgenticIdeasPanel token={token} companyId={Number(companyId)} mode="consultant" />
          ) : null}
        </Card>
      )}

      <ConsultantChatDrawer companyId={Number(companyId)} open={chatOpen} onOpenChange={setChatOpen} />
    </div>
  );
}
...
function AskCompanyAdminPanel({
  ...
  const load = () => {
    if (!token) return;
    api
      .consultantOutreaches(token, companyId)
      .then((d) => {
  ...
  const submit = async () => {
    ...
    setNotice(null);
    try {
      await api.createConsultantOutreach(token, companyId, {
        body: body.trim(),
