...
export function CompanySettings() {
  ...
  const toggleConsultantContact = async () => {
    if (!token || savingConsultant) return;
    const next = !consultantCanContact;
    setConsultantCanContact(next); // optimistic
    setSavingConsultant(true);
    try {
      await api.updateCompanySettings(token, { consultant_can_contact_employees: next });
    } catch {
      setConsultantCanContact(!next); // revert
      setLoadError('Could not update consultant access.');
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card title="Consultant access">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <p className="m-0 font-medium text-foreground">Let your consultant message employees</p>
            <p className="m-0 mt-1 text-sm text-muted-foreground">
              When on, your assigned expert consultant can send a WhatsApp follow-up directly to an employee to
              clarify their answers while reviewing your report. Turn off to require every consultant question to
          ...
          <button
            type="button"
            role="switch"
            aria-checked={consultantCanContact}
            aria-label="Let your consultant message employees"
            disabled={savingConsultant}
            onClick={toggleConsultantContact}
            className={`relative inline-flex h-6 w-11 shrink-0 items-center rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 disabled:opacity-60 ${
              consultantCanContact ? 'bg-primary' : 'bg-muted'
            }`}
          >
            <span
              className={`inline-block h-5 w-5 transform rounded-full bg-card shadow transition-transform ${
                consultantCanContact ? 'translate-x-5' : 'translate-x-0.5'
              }`}
            />
          </button>
        </div>
