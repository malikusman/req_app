...
RSpec.describe "Platform company profile + reviewer cards", type: :request do
  ...
  describe "reviewer public_card" do
    let(:reviewer) do
      create(
        :reviewer_user,
        status: "active",
        ...
        r.reviewer_experiences.create!(organization: "Acme", title: "Director", start_year: 2018, sort_order: 0)
      end
    end

    it "includes bio and experiences on company expert reviewers" do
      ...
      expect(response).to have_http_status(:ok)
      card = JSON.parse(response.body)["expert_reviewers"].first
      expect(card["bio"]).to eq(reviewer.bio)
      expect(card["experiences"].first["organization"]).to eq("Acme")
      expect(card["industries"]).to include("Manufacturing")
    end
  end

  describe "GET /api/v1/platform/reviewers/:id/cv" do
    let(:reviewer) { create(:reviewer_user, status: "active", cv_storage_key: nil) }

    it "returns 404 when no CV" do
      get "/api/v1/platform/reviewers/#{reviewer.id}/cv", headers: headers
      expect(response).to have_http_status(:not_found)
    end

    it "streams CV when present" do
      reviewer.update!(cv_storage_key: "reviewers/#{reviewer.id}/cv/test.pdf")
  ...
  describe "GET /api/v1/platform/reviewers/:id" do
    let(:reviewer) { create(:reviewer_user, status: "active", cv_storage_key: "reviewers/1/cv/x.pdf") }

    it "returns detailed profile with has_cv and platform cv_url" do
      get "/api/v1/platform/reviewers/#{reviewer.id}", headers: headers
      expect(response).to have_http_status(:ok)
      body = JSON.parse(response.body)["reviewer"]
      expect(body["has_cv"]).to eq(true)
      expect(body["profile"]["cv_url"]).to eq("/api/v1/platform/reviewers/#{reviewer.id}/cv")
    end
  end
end
