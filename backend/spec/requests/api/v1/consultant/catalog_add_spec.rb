...
RSpec.describe "Consultant catalog add-from-catalog", type: :request do
  ...
  it "adds a catalog product to the company with consultant attribution" do
    expect do
      post "/api/v1/consultant/companies/#{company.id}/catalog/add",
    ...
    expect(match.added_by_consultant).to eq(consultant)
  ...
  it "blocks adding for an unassigned company" do
    ...
    post "/api/v1/consultant/companies/#{other.id}/catalog/add",
         params: { solution_catalog_entry_id: first_party.id }, headers: auth_headers_for(consultant)
