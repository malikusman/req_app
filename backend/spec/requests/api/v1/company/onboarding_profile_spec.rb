  ...
  context "when the company questionnaire_version is 2" do
    ...
    it "drops a sidecar key for a field that isn't with_other" do
      patch "/api/v1/company/onboarding/questionnaire",
            params: {
              questionnaire_answers: {
                q29_external_parties_channels_other: "Should not persist"
              }
            },
            headers: auth_headers_for(user),
            as: :json
    ...
    it "rejects an over-length sidecar value with a 422 and does not save it" do
      ...
      patch "/api/v1/company/onboarding/questionnaire",
            params: {
              questionnaire_answers: {
                q01_primary_industry_other: too_long
              }
            },
            headers: auth_headers_for(user),
            as: :json
    ...
    it "does not change completion_percent when a sidecar value is present" do
      base_answers = {
        q01_primary_industry: "Logistics & Transportation",
        q02_business_description: "Freight forwarding and warehousing",
        q03_employee_count: "51–100"
      ...
      patch "/api/v1/company/onboarding/questionnaire",
            params: { questionnaire_answers: base_answers },
            headers: auth_headers_for(user),
            as: :json
      percent_without_sidecar = JSON.parse(response.body)["completion_percent"]

      patch "/api/v1/company/onboarding/questionnaire",
            params: {
              questionnaire_answers: base_answers.merge(q01_primary_industry_other: "Some detail")
            },
            headers: auth_headers_for(user),
            as: :json
      percent_with_sidecar = JSON.parse(response.body)["completion_percent"]

      expect(percent_with_sidecar).to eq(percent_without_sidecar)
    end
  end
end
