...
class CompanyClarificationQuestion < ApplicationRecord
  ...
  belongs_to :dismissed_by_reviewer_user, class_name: "ReviewerUser", optional: true

  STATUSES = %w[pending_rag auto_answered open answered dismissed_by_reviewer stale].freeze
  ...
  scope :for_reviewer, -> { where.not(status: "pending_rag") }
end
