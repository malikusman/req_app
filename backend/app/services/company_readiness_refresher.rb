...
class CompanyReadinessRefresher
  ...
  def call
    ...
    departments = @company.employees.where(participation_status: "completed").where.not(department: [nil, ""]).distinct.count(:department)
    ...
    portal_documents = @company.documents.where(status: "ready").count
