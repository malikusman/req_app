...
module Reviewers
  class QuestionnaireProgress
    ...
    def self.call(answers, reviewer: nil)
      new(answers, reviewer: reviewer).call
    end

    def initialize(answers, reviewer: nil)
    ...
    def answered?(id)
      case id
      when "photo"
        @reviewer&.avatar_storage_key.present? || @answers["photo"].present?
      when "name"
        @reviewer&.name.present? || @answers["name"].to_s.strip.present?
      when "email"
        @reviewer&.email.present? || @answers["email"].to_s.strip.present?
      when "cv_upload"
        @reviewer&.cv_storage_key.present? || @answers["cv_upload"].present?
      when "experiences"
    ...
    def experiences_answered?
      ...
      @reviewer&.reviewer_experiences&.any? || false
    end
