    ...
    module Consultant
      class InfoRequestsController < BaseController
        def index
          ...
          requests = policy_scope(::ConsultantInfoRequest).where(company_id: company.id).order(created_at: :desc)
          outreaches = ::ConsultantOutreach
            .where(company_id: company.id, consultant_user_id: current_consultant_user.id)
        ...
        def show
          request = policy_scope(::ConsultantInfoRequest).find(params[:id])
        ...
        def create
          ...
          authorize ConsultantInfoRequest.new(company: company), :create?
        ...
        def thread
          ...
          requests = policy_scope(::ConsultantInfoRequest)
            .where(company_id: company.id, employee_id: employee.id, consultant_user_id: current_consultant_user.id)
            .includes(:consultant_info_replies)
        ...
        def thread_json(r)
          request_json(r).merge(
            replies: r.consultant_info_replies.order(:received_at).map { |reply|
              {
                id: reply.id,
                body: reply.body,
