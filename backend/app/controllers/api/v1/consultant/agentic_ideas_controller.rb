    ...
    module Consultant
      class AgenticIdeasController < BaseController
        ...
        def create
          ...
          idea.source = "consultant"
          idea.status = idea.status.presence_in(%w[draft published]) || "draft"
          idea.created_by_type = current_consultant_user.class.name
          idea.created_by_id = current_consultant_user.id
        ...
        def update
          ...
          idea.updated_by_type = current_consultant_user.class.name
          idea.updated_by_id = current_consultant_user.id
        ...
        def publish
          idea = @company.agentic_ideas.find(params[:id])
          idea.publish!(actor: current_consultant_user)
          render json: { agentic_idea: idea_json(idea) }
        end
