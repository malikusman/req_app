    ...
    module Consultant
      class ReviewSyncController < BaseController
        def show
          ...
          reviews = report.report_reviews.includes(:consultant_user, :report_review_comments, :report_review_section_states)
          ...
          render json: {
            ...
            reviews: reviews.map { |r| { consultant_user_id: r.consultant_user_id, status: r.status, submitted_at: r.submitted_at } },
            ...
            section_states: reviews.flat_map { |r|
              r.report_review_section_states.map { |s|
                { consultant_user_id: r.consultant_user_id, section_key: s.section_key, status: s.status }
              }
            }
          }
        end
